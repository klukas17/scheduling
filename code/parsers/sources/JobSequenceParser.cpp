...
void JobSequenceParser::calculatePredecessorsAndRemainingProcessingTimes(Job* job, PathNode* path_node) {
    ...
    auto processing_time_on_machine = job->getProcessingTime(machine_id);
    ...
    case PARALLEL_GROUP_TOPOLOGY_ELEMENT: {
            ...
            if (next.empty()) {
                parallel_group_path_node->setRemainingProcessingTime(processing_time_on_machine);
            } else {
                ...
                parallel_group_path_node->setRemainingProcessingTime(processing_time_on_machine + remaining_processing_time_in_children);
            }
            break;
    }
