...
MachineType::MachineType(long id, bool preempt, std::vector<std::tuple<long, long, long>> setup_rules_raw_data, std::vector<std::tuple<long, long>> batch_processing_scenario_rules_raw_data) {
    this->id = id;
...
BatchProcessingScenarioRules *MachineType::getBatchProcessingScenarioRules() {
...
void MachineType::constructSetupAndBatchRules(JobTypeMap *job_type_map) {
    ...
    for (auto entry: batch_processing_scenario_rules_raw_data) {
        ...
        long jobs_per_batch = std::get<1>(entry);
